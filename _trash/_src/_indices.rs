...
impl Computer {
    ...
    fn try_compute(&mut self, key: Key) -> PolarsResult<Value> {
        ...
        // Compute
        lazy_frame = compute(lazy_frame, key, length(&key.frame)?)?;
        // Format
...
fn compute(mut lazy_frame: LazyFrame, key: Key, length: usize) -> PolarsResult<LazyFrame> {
    // Calculate
    // let iter = |expr: Expr| {
    ...
    //     })
    // };
    // println!("lazy_frame I 0: {}", lazy_frame.clone().collect().unwrap());
    lazy_frame = lazy_frame.with_columns(
        STEREOSPECIFIC_NUMBERS
            .into_iter()
            .flat_map(|stereospecific_number| {
                (0..length).map(move |index| {
                    col(stereospecific_number)
                        .struct_()
                        .field_by_name("Array")
                        .arr()
                        .get(lit(index as IdxSize), false)
                        .alias(&format!("{stereospecific_number}[{index}]"))
                })
            })
            .collect::<Vec<_>>(),
    );
    // println!("lazy_frame I 1: {}", lazy_frame.clone().collect().unwrap());
    lazy_frame = lazy_frame.select([
        monounsaturated(col(r#"^.*\[\d+\]$"#)),
        polyunsaturated(col(r#"^.*\[\d+\]$"#)),
        saturated(col(r#"^.*\[\d+\]$"#)),
        trans(col(r#"^.*\[\d+\]$"#)),
        unsaturated(col(r#"^.*\[\d+\]$"#), None),
        unsaturated(col(r#"^.*\[\d+\]$"#), NonZeroI8::new(-9)),
        unsaturated(col(r#"^.*\[\d+\]$"#), NonZeroI8::new(-6)),
        unsaturated(col(r#"^.*\[\d+\]$"#), NonZeroI8::new(-3)),
        unsaturated(col(r#"^.*\[\d+\]$"#), NonZeroI8::new(9)),
        eicosapentaenoic_and_docosahexaenoic(col(r#"^.*\[\d+\]$"#)),
        fish_lipid_quality(col(r#"^.*\[\d+\]$"#)),
        health_promoting_index(col(r#"^.*\[\d+\]$"#)),
        hypocholesterolemic_to_hypercholesterolemic(col(r#"^.*\[\d+\]$"#)),
        index_of_atherogenicity(col(r#"^.*\[\d+\]$"#)),
        index_of_thrombogenicity(col(r#"^.*\[\d+\]$"#)),
        linoleic_to_alpha_linolenic(col(r#"^.*\[\d+\]$"#)),
        polyunsaturated_6_to_polyunsaturated_3(col(r#"^.*\[\d+\]$"#)),
        polyunsaturated_to_saturated(col(r#"^.*\[\d+\]$"#)),
        unsaturation_index(col(r#"^.*\[\d+\]$"#)),
        iodine_value(col(r#"^.*\[\d+\]$"#)),
    ]);
    println!("lazy_frame I 2: {}", lazy_frame.clone().collect().unwrap());
    lazy_frame = lazy_frame.select([all().as_expr().struct_().with_fields(vec![
        STEREOSPECIFIC_NUMBERS.map(|stereospecific_numbers| {
            all().as_expr().struct_().field_by_name(name)
        })
    ])]);
...
fn monounsaturated(expr: Expr) -> Expr {
    as_struct(vec![col(FATTY_ACID).fatty_acid().monounsaturated(expr)]).alias("Monounsaturated")
}

fn polyunsaturated(expr: Expr) -> Expr {
    as_struct(vec![col(FATTY_ACID).fatty_acid().polyunsaturated(expr)]).alias("Polyunsaturated")
}

fn saturated(expr: Expr) -> Expr {
    as_struct(vec![col(FATTY_ACID).fatty_acid().saturated(expr)]).alias("Saturated")
}

fn trans(expr: Expr) -> Expr {
    as_struct(vec![col(FATTY_ACID).fatty_acid().trans(expr)]).alias("Trans")
}

fn unsaturated(expr: Expr, offset: Option<NonZeroI8>) -> Expr {
    let name = match offset {
        Some(offset) => format!("Unsaturated{offset}").into(),
        _ => PlSmallStr::from_static("Unsaturated"),
    };
    as_struct(vec![col(FATTY_ACID).fatty_acid().unsaturated(expr, offset)]).alias(name)
}
...
fn fish_lipid_quality(expr: Expr) -> Expr {
    as_struct(vec![col(FATTY_ACID).fatty_acid().fish_lipid_quality(expr)]).alias("FishLipidQuality")
}
...
fn unsaturation_index(expr: Expr) -> Expr {
    as_struct(vec![col(FATTY_ACID).fatty_acid().unsaturation_index(expr)])
        .alias("UnsaturationIndex")
}
