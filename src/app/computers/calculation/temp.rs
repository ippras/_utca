...
fn compute(key: Key, length: u64) -> PolarsResult<Value> {
    ...
    let stereospecific_numbers = |expr: Expr| -> PolarsResult<Expr> {
        Ok(as_struct(vec![
            concat_arr(
                values(expr.clone())
                    .map(|value| fatty_acid().monounsaturated(value).alias("Monounsaturated"))
                    .collect(),
            )?,
            concat_arr(
                values(expr.clone())
                    .map(|value| fatty_acid().polyunsaturated(value).alias("Polyunsaturated"))
                    .collect(),
            )?,
            concat_arr(
                values(expr.clone())
                    .map(|value| fatty_acid().saturated(value).alias("Saturated"))
                    .collect(),
            )?,
            concat_arr(
                values(expr.clone())
                    .map(|value| fatty_acid().trans(value).alias("Trans"))
                    .collect(),
            )?,
            concat_arr(
                values(expr.clone())
                    .map(|value| fatty_acid().unsaturated(value, None).alias("Unsaturated"))
                    .collect(),
            )?,
            concat_arr(
                values(expr.clone())
            ...
            concat_arr(
                values(expr.clone())
                    .map(|value| {
                        (value * fatty_acid().iodine_value().alias("IodineValue"))
                            .sum()
                            .alias("IodineValue")
                    })
                    .collect(),
